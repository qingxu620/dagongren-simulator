...
function SidebarStats({
  ...
  return (
    <aside className="min-h-0 w-full flex flex-col gap-4 p-4 md:h-full md:p-5">
      <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h1 className={`text-lg font-bold sm:text-xl ${isHallucinationMode ? 'text-violet-700' : 'text-slate-900'}`}>
      ...
      <section className="grid grid-cols-2 gap-3">
        {statConfigs.map((item) => {
          ...
          return (
            <div key={item.key} className="relative rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
              <div className="mb-2 flex items-center justify-between gap-2">
                <div className="flex min-w-0 items-center gap-2 text-sm text-slate-600">
                  <Icon size={15} className="shrink-0 text-sky-500" />
                  <span className="truncate">{item.label}</span>
                </div>
                <div className="relative shrink-0 text-sm font-semibold text-slate-800">
                  <span>
                    {displayValue}
      ...
      {showSystemSection ? (
        <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xs text-slate-500">
            商店、股市和背包入口都在这里。{isVictory ? ' 已通关后交易功能会关闭。' : ''}
          </p>
        </section>
      ) : null}
