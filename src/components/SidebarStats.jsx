  ...
  return (
    <aside className="w-full rounded-2xl border border-slate-200 bg-white p-4 shadow-md md:h-full md:overflow-y-auto md:p-5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h1
              className={`truncate text-lg font-bold sm:text-xl ${
                isHallucinationMode ? 'text-violet-700' : 'text-slate-900'
              }`}
            >
              {isHallucinationMode ? '打̸工̴人̵模̶拟̸器̵' : '打工人模拟器'}
            </h1>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              <span className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-xs text-slate-700">
                🏷️ 帮派：{gameState.faction || '无党派牛马'}
              </span>
              <span
            ...
            <p className="mt-2 text-xs text-slate-500">天赋：{talents.length ? talents.join('、') : '暂无'}</p>
          </div>
        </div>
      </div>
            ...
            <div key={key} className="rounded-xl border border-slate-200 bg-white p-2">
              <div className="mb-1 flex items-center justify-between text-xs text-slate-600">
                ...
                <span className="font-semibold text-slate-800">
                  {key === 'money' ? value.toFixed(2) : Math.trunc(value)}
                </span>
              </div>
      ...
      <button
        ...
        className="mb-3 inline-flex h-11 w-full items-center justify-center gap-1 rounded-xl border border-slate-200 bg-slate-50 text-sm font-medium text-slate-700 transition hover:border-sky-200 hover:bg-sky-50 md:hidden"
      >
        {isMobileExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
        {isMobileExpanded ? '收起详细面板' : '展开详细面板'}
      </button>

      <div className={`${isMobileExpanded ? 'block' : 'hidden'} space-y-6 md:block`}>
        ...
        <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
            <button
              ...
              className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 text-sm font-semibold text-blue-700 transition hover:border-blue-300 hover:bg-blue-100 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400 sm:h-11"
            >
        ...
        <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className={`mt-2 text-xs ${hasApiKey ? 'text-emerald-600' : 'text-slate-500'}`}>
            {hasApiKey
              ? '已保存到本浏览器 localStorage，可直接开始游戏。'
              : '未保存 API Key，发送按钮会保持禁用。'}
          </p>
